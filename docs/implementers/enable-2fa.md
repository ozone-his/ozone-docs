...
1. **Initial Login**: When a user logs in for the first time after 2FA has been enabled, they will be prompted to set up their authenticator app. This involves scanning a QR code or entering a setup key manually. Below is a quick demo of the setup process:
![type:video](../assets/videos/quick-configure-otp-demo.mp4)
2. **Subsequent Logins**: For all subsequent logins, users will be required to enter a one-time code generated by their authenticator app in addition to their username and password. Below is a quick demo of the login process:
![type:video](../assets/videos/quick-otp-demo.mp4)
